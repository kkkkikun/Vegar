...
pub fn sys_mount(
    ...
    // For OSComp compatibility: accept any filesystem type and mount a tmpfs.
    // This allows tests like test_mount to succeed without requiring actual
    // block device partition support (e.g., /dev/vda2).
    // Real tmpfs mounts still work as before.
    let fs = MemoryFs::new();
