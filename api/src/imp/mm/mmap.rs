...
pub fn sys_mmap(
    ...
    let writeable = permission_flags.contains(MmapProt::PROT_WRITE) && map_flags.contains(MmapFlags::MAP_SHARED);
    ...
    if populate {
        ...
        if writeable {
            error!(
                "we don't support PROT_WRITE for mmap with fd yet. file: {}. CNMLGB",
                file.path()
            );
        }
